...
baseUrl = "https://api.bitso.com/v2/"
REQ_TIMEOUT = 10  # seconds


class Bitso(ExchangeABC):
    name = 'bitso'
    fiatcurrency = 'MXN'
    ...
    def submit_request(self, path, params=None, private=False, timedelta=86400, retry=0):
        ...
        if response == '{"error": "Invalid nonce"}' and retry < 10:
            ...
            raise ExchangeError('bitso', message=response)
        return response
    ...
    def create_order(self, amount, price, otype):
        ...
        if otype != 'buy' and otype != 'sell':
            raise ExchangeError(exchange='bitso',
        ...
        raise ExchangeError('bitso', 'unable to create order %r' % data)
    ...
    def get_balance(self, btype='total'):
        ...
        """
        try:
            bitsobal = json.loads(self.submit_request('balance', {}, True))
            if 'btc_balance' not in bitsobal or 'mxn_balance' not in bitsobal:
                raise ExchangeError(exchange='bitso',
                                    message="Bitso balance information unavailable")
        except ValueError as e:
            raise ExchangeError('bitso', '%s %s while sending to bitso get_open_orders' % (type(e), str(e)))

        if btype == 'total':
            total = MultiMoney(Money(bitsobal['btc_balance']), Money(bitsobal['mxn_balance'], currency='MXN'))
            return total
        elif btype == 'available':
            available = MultiMoney(Money(bitsobal['btc_available']), Money(bitsobal['mxn_available'], currency='MXN'))
            return available
        else:
            total = MultiMoney(Money(bitsobal['btc_balance']), Money(bitsobal['mxn_balance'], currency='MXN'))
    ...
    def get_order_book(cls, pair='ignored'):
        ...
        if response and 'bids' in response:
            ...
            raise ExchangeError(exchange='bitso', message=response['error'])
        return None
    ...
    def get_ticker(cls, pair='ignored'):
        try:
            ...
            raise ExchangeError('bitso', '%s %s while sending get_ticker to bitso' % (type(e), str(e)))
...
eclass = Bitso
exchange = Bitso(exchange_config['bitso']['api_creds']['key'],
                    exchange_config['bitso']['api_creds']['secret'],
                    exchange_config['bitso']['api_creds']['clientid'])
