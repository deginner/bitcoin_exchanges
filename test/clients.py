...
class TestAPI(unittest.TestCase):
    def test_ticker(self):
        for name, exchange in EXCHANGE.iteritems():
            ...
            if name not in ('btcchina', 'kraken'):  # These do not implement the ticker timeout in the same way
                exchange.REQ_TIMEOUT = 0.0001
                self.assertRaises(ExchangeError, exchange.get_ticker)
                exchange.REQ_TIMEOUT = 10

    def test_get_balance(self):
        for name, exchange in EXCHANGE.iteritems():
            print "test_get_balance %s" % name
            total = exchange.get_balance(btype='total')
            self.assertIsInstance(total, MultiMoney)

            avail = exchange.get_balance(btype='available')
            self.assertIsInstance(avail, MultiMoney)

            result = exchange.get_balance(btype='both')
    ...
    def test_create_order(self):
        for name, exchange in EXCHANGE.iteritems():
            ...
            bal = exchange.get_balance(btype='available')
            if bal.getMoneys(exchange.fiatcurrency) >= Money(1, currency=exchange.fiatcurrency):
                oid = exchange.create_order(amount=1, price=1, otype='bid')
                self.assertIsInstance(oid, str)
            else:
                print "insufficient balance to test create bid order for %s" % name

            self.assertRaises(ExchangeError, exchange.create_order, amount=100000000, price=10,
                              otype='bid')

            if bal.getMoneys('BTC') >= Money(0.01):
                oid = exchange.create_order(amount=0.01, price=ask_price, otype='ask')
                self.assertIsInstance(oid, str)
            else:
                print "insufficient balance to test create ask order for %s" % name

            toomuch = bal.getMoneys('BTC') * 2
            self.assertRaises(ExchangeError, exchange.create_order, amount=float(toomuch.amount), price=ask_price,
                              otype='ask')

    def test_z_cancel_orders(self):
        for name, exchange in EXCHANGE.iteritems():
            print "test_z_cancel_orders %s" % name
            resp = exchange.cancel_orders()
    ...
    def test_order_book(self):
        for name, exchange in EXCHANGE.iteritems():
            ...
            raw_book = exchange.get_order_book()
            ...
            for i in raw_book['asks']:
                item = exchange.format_book_item(i)
